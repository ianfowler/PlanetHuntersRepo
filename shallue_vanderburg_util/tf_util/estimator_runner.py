...
def evaluate(estimator, eval_args):
  ...
  if not checkpoint:
    ...
    logger.log(1, "No checkpoint in {}, skipping evaluation.".format(estimator.model_dir))
    return global_step, values

  logger = tf.get_logger()
  logger.log(1, "Starting evaluation on checkpoint {}".format(checkpoint))
  try:
    ...
    # in some cases.
    logger = tf.get_logger()
    logger.log(1, "Checkpoint {} no longer exists, skipping evaluation.".format(checkpoint))
...
def continuous_train_and_eval(estimator,
  ...
  while True:
    ...
    logger = tf.get_logger()
    logger.log(1, "Starting training at global step {}".format(global_step))
