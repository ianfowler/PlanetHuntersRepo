...
from shallue_vanderburg_util.astronet.ops import dataset_ops
from shallue_vanderburg_util.astronet.ops import metrics
from shallue_vanderburg_util.astronet.ops import training
...
class _InputFn(object):
  ...
  def __call__(self, config, params):
    ...
    # the config type.
    use_tpu = isinstance(config, tf.compat.v1.estimator.tpu.RunConfig)
...
def create_estimator(model_class,
  ...
  if run_config is None:
    run_config = tf.compat.v1.estimator.tpu.RunConfig() # tf.estimator.RunConfig()
  else:
  ...
  use_tpu = isinstance(run_config, tf.compat.v1.estimator.tpu.RunConfig)
