...
import logging
from shallue_vanderburg_util.astronet.data.preprocess import preprocess
...
def _process_file_shard(tce_table, file_name, kdd):
  ...
  logger.log(1, "{}: Processing {} items in shard {}".format(process_name, shard_size, shard_name))
  ...
  with tf.compat.v1.python_io.TFRecordWriter(file_name) as writer:
    ...
    for _, tce in tce_table.iterrows():
      ...
      if not num_processed % 10:
        logger.log(1, "{}: Processed {}/{} items in shard {}".format(process_name, num_processed, shard_size, shard_name))

  logger.log(1, "{}: Wrote {} items in shard {}".format(process_name, shard_size, shard_name))
...
def gen_train_files(exohome, kepler_data_dir, avoid=[], isolate=None, typ="traditional", shards=8, worker_processes=5):
    ...
    np.random.seed(123)
    tce_table = tce_table.iloc[np.random.permutation(num_tces)]
    for bad_kepid in avoid:
        tce_table = tce_table[tce_table.kepid != bad_kepid]
        
    if isolate != None:
        tce_table = tce_table[tce_table.kepid.isin(isolate)]
